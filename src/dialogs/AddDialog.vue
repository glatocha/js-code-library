  ...
  <div id="addDialog" class="z-50 p-5 bg-white border-primary-dark border-2 rounded-lg w-9/12">
    <form @submit.prevent="addItem" class="flex flex-col" autocomplete="off">
      ...
      <div class="flex flex-row justify-end">
        <button
          class="mx-1 text-text font-bold bg-primary px-3 py-2 rounded-lg hover:bg-primary-light hover:text-text-light focus:outline-primary-dark scale-50 sm:scale-100 transition-scale hover:scale-105 duration-50 ease-in-out"
          type="button"
...
export default {
  ...
  setup() {
    ...
    async function addItem() {
      ...
      try {
        const { error } = await supabase.from("javascript-snippets").insert([
          {
